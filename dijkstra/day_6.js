class MinHeap {
  ...
  bubbleUp() {
    ...
    while(idx > 0) {
      ...
      if (this.items[parentIdx][0] <= this.items[idx][0]) break;
  ...
  bubbleDown() {
    ...
    while(idx * 2 + 1 < this.size()) {
      ...
      if (rightChild < this.size() && this.items[rightChild][0] < this.items[leftChild][0]) {
        smallerChild = rightChild;
      }
      if (this.items[idx][0] <= this.items[smallerChild][0]) break;
...
function solutions(graph, start) {
  ...
  pq.push([distances[start], start]); // [길이, 시작노드]
